class User < ActiveRecord::Base
  PLAN_NAMES = %w(apartment office office_premium)
  COUNTRY_CODES = {
    "US": {
      "isoCode": 'US',
  ...
  validates :timezone, inclusion: ActiveSupport::TimeZone.zones_map.values.map(&:name), if: proc { |e| e.plan_name.present? }
  validates :phone_inbound, presence: true, if: proc { |e| e.plan_name.present? }
  validates :country_code, presence: true, if: proc { |e| e.plan_name.present? }
